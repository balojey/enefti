...
app.post("/create-challenge-outbound-transfer", async (req, res) => {
  const { amount, destinationAddress, walletId, userId, userToken, tokenId } = req.body
  try {
    const challengeId = await createChallengeForOutboundTransfer(amount, destinationAddress, walletId, userId, userToken, tokenId);
    res.json({ challengeId });
  } catch (error) {
    console.error(error);
...
async function createChallengeForOutboundTransfer(amount, destinationAddress, walletId, userId, userToken, tokenId) {
